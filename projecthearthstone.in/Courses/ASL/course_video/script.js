...
function saveCompletedVideos(arr) {
  lsSet(LS_VIDEO_PROGRESS, JSON.stringify(arr));
}
function markVideoComplete(videoId) {
  const done = loadCompletedVideos();
  if (!done.includes(videoId)) {
    done.push(videoId);
    saveCompletedVideos(done);
  }
}
function isVideoCompleted(videoId) {
  return loadCompletedVideos().includes(videoId);
}

function buildGlobalVideoList(sections) {
  const list = [];
  sections.forEach((sec) =>
    sec.videos.forEach((v) => list.push({...v, sectionKey: sec.key})),
  );
  return list;
}
...
function applyTheme(section) {
  ...
  document.getElementById("sidebar-section-desc").textContent =
    section.description;
...
function buildSectionTabs(activeSection) {
  ...
  courseData.sections.forEach((sec) => {
    ...
    btn.className =
      "section-tab" + (isActive ? " active" : "") + (isLocked ? " locked" : "");
...
function buildVideoList(section, activeIdx) {
  ...
  section.videos.forEach((video, i) => {
    ...
    const btn = document.createElement("button");
    btn.className =
      "video-item" + (isActive ? " active" : "") + (!unlocked ? " locked" : "");
...
function loadVideo(section, idx) {
  ...
  if (unlocked) {
    ...
    if (!isVideoCompleted(video.id)) {
      ...
      window._completeBtnTimer = setTimeout(
        () => completeBtnWrap.classList.add("visible"),
        3000,
      );
    }
  } else {
  ...
  document.getElementById("current-video-meta").textContent =
    `${section.label} · ${video.duration}`;
  ...
  const nextUnlocked =
    nextExists &&
    isVideoUnlocked(globalVideoList[globalIdx + 1].id, globalVideoList);
  ...
  if (nextExists) {
    ...
    if (nextVid.sectionKey !== section.key) {
      const nextSec = courseData.sections.find(
        (s) => s.key === nextVid.sectionKey,
      );
...
function showToast(text, section) {
  ...
  const [r, g, b] = section
    ? section.portalColor.split(",").map(Number)
    : [170, 85, 255];
...
function switchSection(section) {
  ...
  const targetIdx =
    savedIdx >= 0 && section.key === lsGet("asl_current_section")
      ? savedIdx
      : 0;
...
document.getElementById("btn-prev-vid").addEventListener("click", () => {
  const globalIdx = globalVideoList.findIndex(
    (v) => v.id === currentSection.videos[currentVideoIdx].id,
  );
  if (globalIdx > 0) {
...
document
  ...
  .addEventListener("click", completeCurrentLesson);
...
async function init() {
  ...
  let section =
    courseData.sections.find((s) => s.key === (urlSection || savedSection)) ||
    courseData.sections[0];
  if (!isVideoUnlocked(section.videos[0].id, globalVideoList))
    section = courseData.sections[0];
  let vidIdx = 0;
  if (savedVideo) {
    const found = section.videos.findIndex((v) => v.id === savedVideo);
    if (
      found >= 0 &&
      isVideoUnlocked(section.videos[found].id, globalVideoList)
    )
      vidIdx = found;
  }
  applyTheme(section);
